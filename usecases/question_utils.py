...
from crowdsource_utils import crowdsource_question, checkIfCrowdsource
...
def question(q):
    ...
    elif "picture" in q.lower() or "image" in q.lower():
        return multimedia_question(q)
    elif checkIfCrowdsource(q):
        return crowdsource_question(q)
    else:
        doc = nlp(q)
        for token in doc:
            if token.dep_ == "ROOT":
